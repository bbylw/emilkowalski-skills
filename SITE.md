...
- 目标形态：GitHub Pages（自定义域）。`astro.config.mjs` 的 `site` 与 `public/robots.txt` 里的 sitemap 地址目前是占位域 `skills.emilkowalski.dev`，**上线前必须改成实际分配的域名**，否则 canonical / sitemap / og:url 全错。
- 发布前置：先本地构建 → 用户预览验收 → 明确同意后才建仓、push、配域（团队铁律，勿绕过）。
- 本地预览统一走 portless（命名 HTTPS 域名），无头验收直连 `127.0.0.1:4321` 源端口。
